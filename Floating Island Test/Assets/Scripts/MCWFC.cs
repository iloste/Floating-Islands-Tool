...
public class MCWFC : MonoBehaviour
{
    ...
    //        if (currentCell.coords == new Vector3Int(2, 2, 2))
    //        {

    //        }
    //        else if (currentCell.coords == new Vector3Int(2, 1, 2))
    //        {
    ...
    //            {
    //                if (neighbours[i].coords == new Vector3Int(2, 2, 2))
    //                {

    //                }
    //                else if (neighbours[i].coords == new Vector3Int(2, 1, 2))
    //                {
    ...
    //                    //  bool addToStack = FUNCTIONNEEDSANAME(neighbours[i], validConnections, i);

    //                    if (!q.Contains(neighbours[i]))// to do: add to stack if not collapsed and maybe only if not already in the stack
    //                    {
    //                        if (!neighbours[i].visited && neighbours[i].Collapsed())
    //                        {
    ...
    //        //if (!grid.GridCollapsed())
    //        //{
    //        //    q.Enqueue(grid.FindUncollapsedCell());
    //        //    grid.CollapsePossibleTiles(q.Peek().coords);
    //        //}
    //    }
    //}
    ...
    //                    if (!neighbours[i].visited)
    //                    {
    //                        MCCell[] neighbours2 = grid.GetNeighbouringCells(neighbours[i]);
    ...
    //                                test++;
    //                            }
    //                            else if (neighbours2[x].Collapsed())
    //                            {
    //                                test++;
    //                            }
    //                        }
    ...
    private bool FUNCTIONNEEDSANAME(MCCell cell, List<Connection> validConnections, int direction)
    {
        List<MCTile> possibleNeighbourTiles = cell.possibleTiles;
        bool addToStack = false;

        for (int j = 0; j < possibleNeighbourTiles.Count; j++)
        {
            Connection[] possibleSockets = possibleNeighbourTiles[j].sockets;
            int index = (GetOppositeDirection(direction));

            // if possible tile doesn't match, remove it
            if (!CheckSocketsMatch(possibleSockets[index], validConnections))
            {
                cell.RemovePossibleTile(j);
                j--;
                addToStack = true;//true if something has changed
            }
        }
