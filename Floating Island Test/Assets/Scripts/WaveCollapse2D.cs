...
public class WaveCollapse2D : MonoBehaviour
{
    ...
    private void InitialiseTiles()
    {
        ...
        {
            AddPossibleConnections(allTiles[i]);
        }
    }
    ...
    private void GenerateTiles()
    {
        ...
        {
            allTiles.Add(originalTiles[i]);

            for (int j = 1; j < 4; j++)
            {
                allTiles.Add(CreateRotatedTile(originalTiles[i], j));
            }
        }
    }
    ...
    private void AddPossibleConnections(Tile tile)
    {
